...
from .pag import Causal_Classifier,PaG
...
class GuidedMoEBasic(nn.Module):
    ...
    def get_pair_embedding2(self, emotion_prediction, input_ids, attention_mask, token_type_ids, speaker_ids):
        batch_size, max_doc_len, max_seq_len = input_ids.shape
        print("********line75:",input_ids.shape)
        ...
        # utterance_representation.shape:[batch_size*31,768]
        utterance_representation = self.dropout(pooled_output)
    ...
    def get_pair_embedding(self, emotion_prediction, input_ids, attention_mask, token_type_ids, speaker_ids,mask_ids):
        '''
        utterance_representation.shape:[batch_size*31,768]
        concatenated_embedding.shape:[batch_size*31,776]
        utter_emb.shape:
        mask_ids.shape[batchsize,31,31]
        ...
        _, pooled_output = self.bert(input_ids=input_ids.view(-1, max_seq_len), attention_mask=attention_mask.view(-1, max_seq_len), token_type_ids=token_type_ids.view(-1, max_seq_len), return_dict=False)
        utterance_representation = self.dropout(pooled_output)
        # concatenated_embedding:[batch_size*31,776]
        concatenated_embedding = torch.cat((utterance_representation, emotion_prediction, speaker_ids.view(-1).unsqueeze(1)), dim=1) # 여기서 emotion_prediction에 detach를 해야 문제가 안생기겠지? 해보고 문제생기면 detach 고고
        #concatenated_embedding:[batch_size,31,776]
        concatenated_embedding = concatenated_embedding.view(batch_size,-1,concatenated_embedding.shape[-1])
        # utter_emb:[batch_size,31,776]
